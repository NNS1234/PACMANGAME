...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        #IF NO ACTION RETURN NONE 
        if len(self.getLegalActions(state)) == 0:
            return None
        
        #APPEND ALL Q VAL TO ACTION
        else:
            for action in actions:
                allActions.append((self.getQValue(state, action), action))
            #GET THE PAIRS WHICH ARE BEST
            bestActions = [
                pair for pair in allActions if pair == max(allActions)]
             #RANDOMLY CHOICING  A Q VAL
