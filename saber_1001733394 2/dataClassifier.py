...
def enhancedFeatureExtractorDigit(datum):
    ...
    # Loop all image pixel
    ##Loop for all y in height of digit datum


    for i in range(DIGIT_DATUM_HEIGHT):

        Pixelfreq = feature = pix = otherFeat = 0
    #Loop for all y in width of digit datum
        for j in range(DIGIT_DATUM_WIDTH):

          counter = counter + 1
          k = datum.getPixel(i, j)
          grey = k == 0 #if k==0 gray/black
          white = k > 0 #if k>0 it is white
          if feature == 0 and white: #condition in binary values on feature
            feature = 1

          if pix == 0 and feature == 1 and grey: #that takes 3 binary values
            pix = 1

          if feature == 1 and pix == 1 and white: #binary values features
            otherFeat = 1

          if pix == 1 and otherFeat == 1 and feature == 1 and grey: #takes  3 values
            Pixelfreq = Pixelfreq + 1
            feature = pix = otherFeat = 0

          if Pixelfreq > 0:  # if datum white
            features[counter] = 1
          else:
            features[counter] = k #if gray/black
...
def enhancedPacmanFeatures(state, action):
    ...
    # Generate successor for current state


    closestFood = 0

    if foodList: # find the food in min distance
        closestFood = min([util.manhattanDistance(i, pacPosition) for i in foodList])
    if closestFood:
        closestFood = -1.0 / closestFood #giving weight
 

    else:
        closestFood

    features['closestFood'] = closestFood #labeling and update



    return features #get new feature
...
def readCommand(argv):
    ...
    parser.add_option('-g', '--agentToClone',
                      help=default("Pacman agent to copy"), default=None, type="str")
...
def runClassifier(args, options):
    ...
    if(options.data == "pacman"):
        ...
        trainingData = trainingData or args.get(
            'trainingData', False) or MAP_AGENT_TO_PATH_OF_SAVED_GAMES['ContestAgent'][0]
        validationData = validationData or args.get(
            'validationData', False) or MAP_AGENT_TO_PATH_OF_SAVED_GAMES['ContestAgent'][1]
