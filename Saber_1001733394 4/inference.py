...
class ExactInference(InferenceModule):
      ...
      #  print pacmanPosition
        ...
        for p in self.legalPositions:
            ...
            if noisyDistance is None:
                   ...
                   allPossible[self.getJailPosition()] = 1.0 #update jail
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        #util.raiseNotDefined()
        new_self = util.Counter()
        prob=1.0
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos)) #from the above hint
            #print newPosDist.items()
            for newPos, prob in newPosDist.items():
                new_self[newPos] += prob*self.beliefs[oldPos]

        self.beliefs = new_self
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while t is not 1  :
            for p in (self.legalPositions):
                if  len(self.particles)== self.numParticles: #evenly distribute
                    t = 1
                    continue
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for i in range(0, self.numParticles):
               self.particles.append(self.getJailPosition()) #case  1
        else :

          for pos in self.particles:
             trudist = util.manhattanDistance(pacmanPosition,pos)
             weight[pos] += emissionModel[trudist]

          weight.normalize()

          if weight.totalCount() == 0:
             self.initializeUniformly(gameState)#case 2
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(
                self.setGhostPosition(gameState, oldPos))  # to obtain the distribution over new positions for the ghost, given its..
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        #util.raiseNotDefined()
        new_bel = util.Counter()

        for _ in (self.legalPositions):
          for p  in self.particles:
              new_bel[p] = new_bel[p]+1
        new_bel.normalize()
        self.belief = new_bel
        #print self.belief
        return new_bel
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        prod = itertools.product(
            self.legalPositions, self.legalPositions)

        posspositions = []
        for item in prod:
            posspositions.append(item)
        count=0
        self.particles = []
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # compute weights for each particle then resample particles from weighted distribution
        ...
        self.beliefs=weightedParticles
    ...
    def getBeliefDistribution(self):
        ...
        for p in belief:
            belief[p] = (1.0*belief[p])/(1.0*self.numParticles)
        self.belief = belief
